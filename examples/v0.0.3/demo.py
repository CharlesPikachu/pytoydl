'''
Function:
    "从零开始实现一个深度学习框架 | 常用优化算法介绍与实现"完整示例代码
Author:
    Zhenchao Jin
微信公众号:
    Charles的皮卡丘
...
# 命令行参数解析
def parseArgs():
    parser = argparse.ArgumentParser(description='不同优化算法比较')
    parser.add_argument('--optim', dest='optim', help='优化算法名称', type=str, required=False)
...
# 定义优化器
optimzer = getattr(toydl.optim, args.optim)(cnn, criterion=criterion)
# 导入数据集
# --手写数字数据集
