'''
Function:
    “从零开始实现一个深度学习框架 | 打造基础学习框架篇”完整示例代码
Author:
    Zhenchao Jin
微信公众号:
    Charles的皮卡丘
...
# 定义模型
model = nn.Sequential()
model.addmodule('fc1', nn.Linear(64, 128))
model.addmodule('fc2', nn.Linear(128, 10))
model.addmodule('softmax', nn.Softmax())
# 定义损失函数
criterion = nn.CrossEntropy()
# 定义优化器
optimzer = toydl.optim.SGD(model, criterion=criterion, learning_rate=0.001, momentum=0.9)
# 导入数据集
# --手写数字数据集
...
for epoch in range(60):
    ...
    predictions = model(valset[0]).argmax(1)
